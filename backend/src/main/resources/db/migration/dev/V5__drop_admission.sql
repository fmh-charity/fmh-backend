...
alter table patient
    add if not exists plan_date_in timestamp;
alter table patient
    add if not exists plan_date_out timestamp;
alter table patient
    add if not exists fact_date_in timestamp;
alter table patient
    add if not exists fact_date_out timestamp;
alter table patient
...
set (room_id, plan_date_in, plan_date_out, fact_date_in, fact_date_out, status) =
        (select room_id, plan_date_in, plan_date_out, fact_date_in, fact_date_out, status
         from admission
         where admission.id = patient.current_admission_id);

drop table admission cascade;
alter table patient drop column current_admission_id;